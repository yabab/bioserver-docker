...
mysql -uroot -p$MYSQL_ROOT_PASSWORD <<-EOSQL
  ...
  CREATE DATABASE IF NOT EXISTS \`$BIO2_DB_DATABASE\` DEFAULT CHARACTER SET latin1 COLLATE latin1_swedish_ci;
  GRANT ALL PRIVILEGES ON \`$BIO2_DB_DATABASE\`.* TO '$MYSQL_USER'@'%'; 

  USE \`$BIO2_DB_DATABASE\`;
